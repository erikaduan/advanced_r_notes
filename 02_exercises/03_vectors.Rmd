...
+ **Strings**   
  ...
  + Strings are surrounded by `"string"` (or `'string'`) quotation marks. Note that characters in strings can be escaped using `\` i.e. using `\n` to create a new line.     
...
Missing values are considered infectious i.e. computations involving missing values return missing values. This is why `is.na()` is required to subset values that are `NA`.   
...
**Note:** `NA` will be automatically coerced to the correct type when needed.   
...
Most mathematical functions will coerce to numeric and this is the reason why functions like `sum()` and `summary()` can count the number of logical elements. 
...
Individual attributes are emphemeral as they are lost when passed through a function. The exceptions are the attributes **dim** (dimensions) and **name**. To preserve other attributes, an individual S3 class needs to be created.   
...
Adding a dim attribute to a vector allows it to behave like a 2-dimensional matrix or a multi-dimensional array. Matrices and arrays can be created with `matrix()` and `array()`, or by using the assignment form of `dim()`.  
...
Equivalent dimension-related functions also exist for vectors, matrices and arrays (diagram from https://adv-r.hadley.nz/vectors-chap.html#attributes).   

```{r, echo = FALSE, results = 'markup', fig.align = 'center', fig.show = 'hold', out.width = '100%'}  
...
Possessing the vector attribute `class` is what turns an object into a S3 object. This also means that it will behave differently when passed to a generic function.  
...
A factor is a vector that can only contain predefined character values. Factors are built on top of an integer vector with two attributes: a “factor” class, which makes it behave differently from regular integer vectors, and levels, which defines the set of allowed values.   
...
Dates are built on top of a double vector (with the double vector representing the number of days since 1970-01-01). Dates have a class "Date" and no other attributes.  
...
POSIXct vectors are also built on top of double vectors, where the value represents the number of seconds (rather than days) since 1970-01-01.   
...
**Note:** The `tzone` attribute controls only how the date-time is formatted i.e. "AEST".      
...
Unlike a data frame, a list can contain vectors of different lengths and types. Each element in a list is a reference that can refer to an object of any type (recalling the chapter on names and values).  

**Note:** Because a list is a vector that contains references to individual objects in memory, the total size of a list can be smaller than expected (i.e. when the same value appears, the references are pointing to the same object in memory).   
...
List-matrices and list-arrays are esoteric object structures but they can be useful if you want to arrange objects in a grid-like structure. For example, if you’re running models on a spatio-temporal grid, it might be more intuitive to store the models in a 3D array that matches the grid structure.  
...
```{r, echo = FALSE, results = 'markup', fig.align = 'center', fig.show = 'hold', out.width = '50%'}  
...
It is important to note that tibbles never coerce their input (this is one feature that makes them lazy). Tibbles also do not transform non-syntactic names (i.e. automatically convert `1` into `X1` as a column name).   
...
You can theoretically get and set unique row names with rownames(), and you can use them to subset rows. This is because data frames share some behaviour with matrices (which only contain numerical values and are transposable).  
...
+ Row names only work when a row can be identified by a single character string (precludes numeric labels or labels with more than one vector i.e. latitude and longitude).  
...
+ When you subset columns with `df[, vars]`, you will get a vector if you are subsetting based on one variable, but a data frame if you are subsetting based on more than one variable.    
  + This is a frequent source of bugs when using [ in a function, unless you always remember to use `df[, vars, drop = FALSE]`.    
...
With tibbles, `df[, vars]` always returns a tibble and a `$` doesn’t do partial matching and warns if it can’t find a variable.   
...
Since a data frame is a list of vectors, it is possible for a data frame to have a column that is a list. This is very useful because a list can contain any other object: this means you can put any object in a data frame.   
...
NULL is special because it has a unique type, is always length zero and can’t have any attributes (i.e. no names or dimensions).     
